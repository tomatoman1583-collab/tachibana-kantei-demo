...
| 内容 | 木立と家々にはさまれた住宅街の細い通り | 大きな窓が並ぶ、家具のない事務所の室内 | 大きな窓ぎわに机と椅子、観葉植物を置いた明るい室内 |
| 撮影者 | Tsuyoshi Kozu | Drew Beamer | Annie Spratt |
| 入手元 | Unsplash（https://unsplash.com/photos/g3TWCAH9Pto） | Unsplash（https://unsplash.com/photos/dbKwY7Ijsvw） | Unsplash（https://unsplash.com/photos/2pia6OAe5rA） |
| 取得日 | 2026-09-06 | 2026-09-06 | 2026-09-06 |
| 解像度・容量 | 2400×1600px／約809KB（JPEG） | 2400×1600px／約438KB（JPEG） | 2400×1600px／約343KB（JPEG） |
| ライセンス | Unsplash License（商用利用可・改変可・クレジット表記任意・再配布不可） | 同左 | 同左 |
| 人物の写り込み | なし | なし | なし |
| 備考 | 原付と自転車が写るが、ナンバープレート・表札・看板など個人や実在の事業者を特定できる文字は判読できないことを確認済み | **全画素が R=G=B の白黒写真**（実測: 最大彩度差0）。カラーの他2点とは色の系統が異なる | 室内の小物のみ。個人を特定できる要素なし |

Unsplash Licenseはクレジット表記が任意のため、サイト本体では表記していません。

### 派生ファイル（16:5クロップ・リサイズ・WebP変換）

原本3点から、Pillow（12.1.1）で **先に 16:5（2400×750）へ切り出してから** リサイズ・再エンコードして生成した派生ファイルです。**内容は原本と同一の写真で、上記と同じUnsplash Licenseの下にあります**（Unsplash Licenseは改変を許諾しています）。原本 `page-*.jpg`（2400×1600）は差し替えず、そのまま保持しています。
...
| `assets/page-about-400.jpg` / `.webp` | 400×125 | 13.4KB | 44.63dB / 1.50 | 8.5KB | 44.53dB / 1.51 |
| `assets/page-about-560.jpg` / `.webp` | 560×175 | 20.3KB | 45.53dB / 1.35 | 11.5KB | 44.83dB / 1.46 |
| `assets/page-about-880.jpg` / `.webp` | 880×275 | 38.7KB | 46.39dB / 1.22 | 18.7KB | 45.38dB / 1.37 |
| `assets/page-about-1200.jpg` / `.webp` | 1200×375 | 62.1KB | 46.63dB / 1.19 | 27.0KB | 45.28dB / 1.39 |
| `assets/page-about-1600.jpg` / `.webp` | 1600×500 | 100.0KB | 47.11dB / 1.13 | 40.8KB | 45.39dB / 1.37 |
...
- 生成方法：Pillow 12.1.1。原本 2400×1600 から `crop((0, y, 2400, y+750))` で 16:5 を切り出し（切り出し上端 y は cases=425 / about=425 / contact=650。写り込ませたい被写体を基準に個別に決定）、`Image.resize(..., Image.LANCZOS)` で縮小。ICCプロファイルは原本から引き継ぎ、全ファイルで `w*5 == h*16` をアサート
...
- **上限を1600pxにした引き換え**：デスクトップ最大表示幅1072pxに対しDPR2で必要なのは2144pxだが、最も重い `page-cases` は2144px幅のWebPで298KBに達する。ヒーロー写真も同じ理由で1600pxを上限にしている（1カラム時の最大852pxに対し1.88倍）ため揃え、**DPR2のデスクトップでは1.49倍相当**にとどまる。写真主体で細い文字を含まない帯であることを踏まえた判断で、2倍を厳密に満たしてはいない
- 品質の根拠：q90 を採用（ヒーロー写真と同一）。30ファイルの実測は **JPEGが 36.88〜47.11dB（RMSE 1.13〜3.65）、WebPが 36.74〜45.39dB（RMSE 1.37〜3.71）**。WebP非対応ブラウザにはJPEGを配信するため、実際に描画されうる値の下限は最小幅400pxの `page-cases` の 36.74dB（WebP）／36.88dB（JPEG）になる
...
- 単調性について：JPEGは3点とも幅が広いほど値が高くなる単調な並び。**WebPは `page-about` だけ 880px（45.38dB）→1200px（45.28dB）で 0.10dB わずかに下がる**（他の2点は単調）。階調の平坦な白黒写真で q90 のWebPが品質の頭打ちに達しているためで、劣化ではなく指標の頭打ち。ヒーロー写真の節の「どちらの形式も単調」という記述は**ヒーロー写真についての実測であり、この3点には当てはまらない**
